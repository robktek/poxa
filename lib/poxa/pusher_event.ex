defmodule Poxa.PusherEvent do
  ...
  def subscription_succeeded(channel) when is_binary(channel) do
    %{event: "pusher_internal:subscription_succeeded", channel: channel, data: %{}} |> encode!
  end
  ...
  def subscription_succeeded(%PresenceSubscription{channel: channel, channel_data: channel_data}) do
    ...
    %{event: "pusher_internal:subscription_succeeded", channel: channel, data: data} |> encode!
  end
  ...
  Returns a JSON for a new member subscription on a presence channel
      ...
      "{ "event" : "pusher_internal:member_added",
         ...
         "data" : { "user_id" : 123,
                    "user_info" : "456" } }"
  """
  @spec presence_member_added(
          binary,
          PresenceSubscription.user_id(),
          PresenceSubscription.user_info()
        ) :: binary
  def presence_member_added(channel, user_id, user_info) do
    data = %{user_id: user_id, user_info: user_info} |> encode!
    %{event: "pusher_internal:member_added", channel: channel, data: data} |> encode!
  end
  ...
  def presence_member_removed(channel, user_id) do
    ...
    %{event: "pusher_internal:member_removed", channel: channel, data: data} |> encode!
  end

  defstruct [:channels, :name, :data, :socket_id, :user_id]

  @type t :: %Poxa.PusherEvent{
          channels: list,
          name: binary,
          user_id: nil | binary,
          data: binary | map,
          socket_id: nil | binary
        }
